...
export const projects: Project[] = [
  {
    ...
    highlights: [
      ...
      'An MCP server exposes the search and the retrieval report as tools, so a model querying the corpus also gets the measured quality of the retriever it just used, and the caveats attached to it.',
      'Eight documented REST endpoints with an OpenAPI schema generated from the serializers. 496 tests and the retrieval gate run on every push.',
    ],
    body: [
      ...
      'The result did not go the way I expected, and it is published as it came out. Dense retrieval leads on recall@10 and nDCG@10; hybrid edges MRR by a margin far inside the noise. A paired bootstrap over 10,000 resamples puts dense ahead of BM25 by 0.218 nDCG@10 (p=0.0001) and hybrid ahead of BM25 by 0.176 (p=0.0001), but hybrid against dense comes out at −0.041 with a confidence interval straddling zero (p=0.194). On this corpus, the fusion step buys latency and complexity without buying accuracy.',
      'One decision worth stating because it went against my own interest. Sweeping the RRF constant k showed that k=5 scored better than the default k=60 on this golden set. I left it at 60. The sweep was run on the same 40 queries the results are reported against, so tuning on them and then quoting the improvement would be fitting the test set and calling it a finding.',
      'Where the modes genuinely diverge is paraphrase. Rewriting each query so it shares no vocabulary with the paper drops BM25 recall@10 to 0.206 while dense holds 0.615, which is the case dense retrieval is actually for. The interface says which mode found each result, so when BM25 surfaces something dense missed entirely, the result carries a note saying so rather than arriving unexplained.',
      'On top of retrieval there is a grounded answering layer, evaluated the same way. Against the 50 golden queries it abstained on all 10 unanswerable ones and produced no fabricated citations; every citation it did emit resolved to a passage that was actually retrieved. It also declined to answer 4 questions that were answerable, which is the trade that abstention buys and is reported alongside the rest.',
      'The finding I did not expect from the answering work: of the four faults I injected to check the evaluation could see them, the ordinary unit tests caught all four before the evaluation harness did. The harness is still worth having as a regression gate, but it is not the thing that catches bugs.',
    ],
    table: {
